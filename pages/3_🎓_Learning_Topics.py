...
answer_button_sb = st.sidebar.button(
    "Get Answer&nbsp;&nbsp;➠", type="primary", key="answer_button_sb")

st.markdown("<br>", unsafe_allow_html=True)

topic = st.text_input("What would you like to learn about?",
                      placeholder="Ask about a coding topic here")
answer_button = st.button("Get Answer&nbsp;&nbsp;➠", type="primary")

if answer_button or answer_button_sb:
    advice = st.markdown("### Ducky Teaching...")
    learning_prompt = services.prompts.learning_prompt(
        learner_level + topic + response_format)
    messages = services.llm.create_conversation_starter(
        services.prompts.system_learning_prompt())
    messages.append({"role": "user", "content": learning_prompt})
    # messages.append({"role": "user", "content": topic})  # Add this line
