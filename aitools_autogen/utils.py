...
def extract_code(
    ...
    for match in matches:
        ...
        full_code = "\n".join(
            filter(None, [filename_line, commented_preamble, code]))
        ...
        if isinstance(item, ast.FunctionDef):
            ...
            for class_item in item.body:
                if (isinstance(class_item, ast.FunctionDef) or isinstance(class_item, ast.AsyncFunctionDef)) and not class_item.name.startswith("_"):
                    ...
                    functions.append(
                        f"{item.name}.{_get_function_signature(class_item)}")
...
def save_code(
    ...
    if not filename:
        ...
        filename = f"tmp_code_{code_hash}.{
            'py' if lang.startswith('python') else lang}"
...
def clear_working_dir(work_dir: Optional[str] = None, filename_wildcard: str = '*.py') -> None:
    ...
    if os.path.exists(work_dir) and os.path.isdir(work_dir):
        ...
        python_files = glob.glob(os.path.join(
            work_dir, '**', filename_wildcard), recursive=True)

        for file_path in python_files:
            try:
                os.remove(file_path)  # Delete the file
                # Optional: printing confirmation
                ...
                # Handle exceptions during file or directory deletion
...
def print_conversable_agent_state(agent: Agent):
    ...
    if isinstance(agent, ConversableAgent):
        ...
        _print_formatted_field("_code_execution_config",
                               agent._code_execution_config)
        _print_formatted_field("_max_consecutive_auto_reply",
                               agent._max_consecutive_auto_reply)
        _print_formatted_field(
            "_consecutive_auto_reply_counter", agent._consecutive_auto_reply_counter)
        _print_formatted_field(
            "_max_consecutive_auto_reply_dict", agent._max_consecutive_auto_reply_dict)
        _print_formatted_field("_is_termination_msg",
                               agent._is_termination_msg)
        _print_formatted_field("_function_map", agent._function_map)
        _print_formatted_field("_default_auto_reply",
                               agent._default_auto_reply)
        ...
        _print_formatted_field("_oai_system_message",
